...
void AssetShader::runAndLink(const char* vertexFile, const char* fragmentFile)
{
    ...
    runTimeErrors(m_Shaders, "PROGRAM");
...
void AssetShader::runTimeErrors(unsigned int tempShader, std::string tempType)
{
    int  itRuns;
    char infoLog[1024];
    if (tempType != "PROGRAM")
