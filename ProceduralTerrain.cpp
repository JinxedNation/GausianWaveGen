...
ProceduralTerrain::ProceduralTerrain(unsigned tempSize) : Terrain(tempSize), m_TerrainData(m_TerrainSize* m_TerrainSize), m_TerrainTexture(nullptr), m_WorldShader(nullptr)
{
...
bool ProceduralTerrain::loadTerrainData(const char* readFileName, unsigned tempSize)
{
    ...
    unsigned numBytes = tempSize * tempSize * sizeof(float);
...
float ProceduralTerrain::getTerrainScaledWidth() const
{
    return m_ScaledWidth;
}


float ProceduralTerrain::getTerrainScaledLength() const
{
    return m_ScaledLength;
}

float ProceduralTerrain::getTerrainScaledHeight() const
{
...
bool ProceduralTerrain::createTerrain(const char* readFileName)
{
    ...
    // Generate terrain
    generateTerrain(m_TerrainWidth, m_TerrainHeight, m_TerrainRoughness, m_NumOctaves, m_TerrainPersistance);
...
void ProceduralTerrain::generateTerrain(int width, int height, float roughness, int numOctaves, float persistence)
{
    ...
    runErosion(m_MapWater, m_TerrainData, 100, m_ErodedMaterialCap, m_ErosionSpeed, 0.5f);
...
void ProceduralTerrain::setTerrainMaterial(const std::string& texturePath)
{
    m_WorldShader[0] = AssetShader("shaders/terrain_vs.shader", "shaders/terrain_fs.shader");
}



void ProceduralTerrain::setTerrainShader(std::shared_ptr<TextureLoader> terrainShader)
{
    m_TerrainTexture = terrainShader;
}
...
void ProceduralTerrain::generateTexture()
{
    if (m_TerrainTexture)
    {
        m_TerrainTexture = std::make_unique<TextureLoader>("Earth.png");
    }
}
...
void ProceduralTerrain::setWaterLevel(float tempLevel)
{
    m_WaterLevel = tempLevel;
}


void ProceduralTerrain::setErosionValues(float tempRadius, float tempDepth, float tempCap)
{
    m_Radius            = tempRadius;
    m_Depth             = tempDepth;
    m_ErodedMaterialCap = tempCap;
}


void ProceduralTerrain::setErosionSpeed(float tempSpeed)
{
    m_ErosionSpeed = tempSpeed;
}


void ProceduralTerrain::setSize(int tempWidth, int tempHeight)
{
    m_TerrainWidth  = tempWidth;
    m_TerrainHeight = tempHeight;
}
...
void ProceduralTerrain::erodeTerrain(int x_Coord, int y_Coord, float amount, std::vector<float>& m_TerrainData, std::vector<float>& m_ErodedTerrain)
{
    const float maxWater = 1.0f;

    if (x_Coord < 0 || x_Coord >= m_TerrainSize || y_Coord < 0 || y_Coord >= m_TerrainSize)
    {
        return;
    }
    ...
    moveErodedMaterial(x_Coord, y_Coord, deposit, m_TerrainData, m_ErodedTerrain);
...
float ProceduralTerrain::calculateMinimum(const std::vector<float>& m_TerrainData) const
{
    ...
    float minimum = m_TerrainData.front();

    for (unsigned i = 0; i < m_TerrainData.size() - 1; i += 2)
    {
        float compareValue = m_TerrainData[i];
        float smallestValue = m_TerrainData[i + 1];

        if (compareValue < smallestValue)
        {
            minimum = std::min(minimum, compareValue);
        }
        else
        {
            minimum = std::min(minimum, smallestValue);
        }
    }
    ...
    {
        minimum = std::min(minimum, m_TerrainData.back());
    }
    return minimum;
}
...
float ProceduralTerrain::calculateMaximum(const std::vector<float>& m_TerrainData) const
{
    ...
    float maximum = m_TerrainData.front();

    for (unsigned i = 0; i < m_TerrainData.size() - 1; i += 2)
    {
        float largest = m_TerrainData[i];
        float compareV = m_TerrainData[i + 1];

        if (largest > compareV)
        {
            maximum = std::max(maximum, largest);
        }
        else
        {
            maximum = std::max(maximum, compareV);
        }
    }
    // If the size of the vector is odd, we need to compare the last value separately
    if (m_TerrainData.size() % 2 == 1)
    {
        maximum = std::max(maximum, m_TerrainData.back());
    }
...
{
    float material = pow(angle, errodedMaterialCap) * water;
...
float ProceduralTerrain::calculateErodedMaterial(float fallAngle, float water, float amount)
{
    ...
    float erodedMaterialCap = (m_TravelAngle * m_Water * m_Gravity) / m_Cohesion;
    float dumpMaterial      = std::min(amount, erodedMaterialCap - water);

    float erosionAngle      = std::max(0.0f, static_cast<float>(std::cos(m_Gravity * std::acos(-1.0) / 180.0)) - m_TravelAngle);
    float cohesionFactor    = m_Cohesion / (m_Water + dumpMaterial);
...
void ProceduralTerrain::setViewMatrix(const glm::mat4& viewMat)
{
    m_ViewMat = viewMat;
}

void ProceduralTerrain::setProjMatrix(const glm::mat4& proj)
{
    m_ProjMat = proj;
}
...
{
    // Bind the texture
    m_TerrainTexture->bindNewTexture(0);

    // Bind the VAO
    glBindVertexArray(m_TerrainVAO);

    // Render the terrain using the index buffer
    ...
    // Unbind the texture
    m_TerrainTexture->unbindOldTexture();
}